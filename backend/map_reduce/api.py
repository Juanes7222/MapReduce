...
def create_app() -> FastAPI:
    ...
    async def create_job(job_data: JobCreate):
        try:
            ...
            return JobResponse(
                job_id=job_id, status='map', text_length=len(text),
                num_shards=num_shards, created_at=job['created_at']
            )
    ...
    async def list_jobs():
        ...
        for job_id, job in coordinator.jobs.items():
            ...
            jobs_list.append(JobResponse(
                job_id=job_id, status=job['status'], text_length=len(job['text']),
                num_shards=job['num_shards'], top_words=job['top_words'],
    ...
    async def list_engines():
        ...
        for engine_id, engine in coordinator.engines.items():
            ...
            engines_list.append(EngineInfo(
                engine_id=engine_id, role=engine['role'], capacity=engine['capacity'],
    ...
    app.add_middleware(CORSMiddleware, allow_origins=_allow_origins, allow_methods=["*"], allow_headers=["*"], allow_credentials=_allow_credentials)
