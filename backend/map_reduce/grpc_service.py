...
class JobServiceServicer(jobs_pb2_grpc.JobServiceServicer):
    def RegisterEngine(self, request, context):
        ...
        coordinator.add_log(f"Engine {engine_id} registrado como {role} con capacidad {capacity}")
        return jobs_pb2.RegisterEngineReply(success=True, message=f"Engine {engine_id} registrado correctamente")
    ...
    def FetchJob(self, request, context):
        ...
        if engine['role'] == 'mapper' and coordinator.map_queue:
            ...
            return jobs_pb2.FetchJobReply(
                task_type="map",
                map_task=jobs_pb2.MapTask(job_id=job_id, shard_id=shard_id, text_content=text)
            )
        ...
        if engine['role'] == 'reducer' and coordinator.reduce_queue:
            ...
            return jobs_pb2.FetchJobReply(
                task_type="reduce",
                reduce_task=jobs_pb2.ReduceTask(job_id=job_id, word=word, counts=counts)
            )
        return jobs_pb2.FetchJobReply(task_type="none")
    ...
    def ReportResult(self, request, context):
        ...
        if engine_id in coordinator.engines:
            ...
            coordinator.engines[engine_id]['current_load'] = max(0, coordinator.engines[engine_id]['current_load'] - 1)
        ...
        if task_type == "map":
            ...
            if job['completed_shards'] == job['num_shards']:
                ...
                coordinator.add_log(f"Job {job_id} pasa a REDUCCIÓN con {job['num_reduce_tasks']} tareas")
            ...
            coordinator.add_log(f"Resultado de reducción recibido de {engine_id} (Trabajo={job_id}, palabra={word}, conteo={total})")
