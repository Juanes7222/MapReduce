...
ENABLE_MONGODB = os.environ.get('ENABLE_MONGODB', 'false').lower() == 'true'
...
if ENABLE_MONGODB:
    ...
    username = os.environ['DB_USERNAME']  # FIX: was 'USERNAME' (Windows env var)
    ...
    logger.info(f"MongoDB enabled, connecting to {host}")
    client = AsyncIOMotorClient(mongo_url)
    db = client[app_name]
else:
    logger.info("MongoDB disabled (ENABLE_MONGODB=false), using in-memory storage only")
    client = None
    db = None
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if ENABLE_MONGODB and client is not None:
            try:
                client.close()
...
async def create_job(job_data: JobCreate):
    try:
        ...
        if ENABLE_MONGODB and db is not None:
            # Fire-and-forget to avoid blocking job creation
            asyncio.create_task(db.jobs.insert_one({
                'job_id': job_id,
